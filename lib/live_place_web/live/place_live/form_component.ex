defmodule LivePlaceWeb.PlaceLive.FormComponent do
  use LivePlaceWeb, :live_component
  ...
  defp save_place(%{assigns: %{current_user: user}} = socket, :new, place_params) do
    case Places.create_place(user, place_params) do
      {:ok, _place} ->
        {:noreply,
         socket
      ...
      {:error, :place, %Ecto.Changeset{} = changeset, _} ->
        {:noreply, assign(socket, changeset: changeset)}
    end
  end
end
